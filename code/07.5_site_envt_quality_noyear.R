...
#
# Rscript code/07.5_site_envt_quality_noyear.R \
...
#   --outfile_base "figure_inputs/sunflower_climate_ranef_noTX_noYr" \
...
run_climate_window_analysis_ranef <- function(day_sub, param_thetas, nrepeats = 100, 
  ...
  formula_str <- paste("theta ~ 1 + (1|county_state)")
  ...
  repeat {
    ...
    cat("forstr")
    formula_str <- paste("theta ~ 1 + (1|county_state) +", signal_terms)
...
run_climate_window_analysis_fixef <- function(day_sub, param_thetas, nrepeats = 5, 
  ...
  # Initial baseline model
  baseline_formula <- lm(theta ~ county_state, data = param_thetas)
  ...
  repeat {
    ...
    formula_str <- paste("theta ~ county_state +", signal_terms)
    Sys.sleep(3)
