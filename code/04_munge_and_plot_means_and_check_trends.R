...
rebuild_trends_plot <- function(trend_data, trend_check_data, custom_order = NULL) {
  library(ggplot2)
  library(dplyr)
  library(patchwork)
  ...
  trend_plot <- ggplot(meta_reg, aes(x = Year)) +
    ...
    scale_fill_identity() +
    facet_wrap(~ factor(State, levels = rev(ordered_states)), ncol = 1, scales = "free_y") +
...
# function to rebuild the trends plots (means and checks)
rebuild_gain_plot <- function(rds_file_path, 
                              new_theme = NULL,
                              new_title = NULL,
                              new_subtitle = NULL,
                              new_x_label = NULL,
  ...
  if (is.null(new_subtitle)) {
    subtitle <- paste("Per-state predictions with 95% CI and slope", pval_text, sep = "\n")
  } else {
...
for (response_var in c("oil_pct","yield_lb_acre","flower_50pct_days_past_planting" ,
  ...
  cat(response_var)
  all_files <-list.files("figure_inputs", pattern = response_var, full.names = TRUE)
...
combined_plot <- (trends$trial_plot+ggtitle("Trial Means") +
                             theme(plot.subtitle = element_text(size = 10))) +
  (gen+ ggtitle ('Genetic') +theme(plot.subtitle = element_text(size = 10)))+
  (env+ggtitle('Environment') +
     theme(plot.subtitle = element_text(size = 10))) +
  (trends$check_plot+ggtitle("Checks") + theme(plot.subtitle = element_text(size = 10)))+
  plot_layout(widths = c(1, 1,1,1))

ggsave(file.path("figures", paste0(response_var,"all_trends.jpg")),
