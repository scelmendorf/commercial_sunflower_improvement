...
for (response_var in c(
  ...
  blue_results <- map_dfr(states, function(st) {
    ...
    # Require at least 10 years of data
  ...
  # wrangle
  if (!is.null(blue_results) && nrow(blue_results) != 0) {
    ...
    if (!use_piepho) {
      write_rds(agronomy_plot_data, file = file.path(
        "figure_inputs",
      ...
      write_rds(breeding_plot_data, file = file.path(
        "figure_inputs",
        paste0(response_var, "_genetic_gain.rds")
      ))

      write.csv(
        meta_reg_agronomy_results %>%
