...
// Function to split a state into four quarters
function splitStateIntoQuarters(state) {
  ...
  // Create four bounding boxes (NW, NE, SW, SE)
  var nwBounds = ee.Geometry.Rectangle([
    minLon, midLat,  // min lon, mid lat
    midLon, maxLat   // mid lon, max lat
  ]);
  
  var neBounds = ee.Geometry.Rectangle([
    midLon, midLat,  // mid lon, mid lat
    maxLon, maxLat   // max lon, max lat
  ]);
  
  var swBounds = ee.Geometry.Rectangle([
    minLon, minLat,  // min lon, min lat
    midLon, midLat   // mid lon, mid lat
  ]);
  
  var seBounds = ee.Geometry.Rectangle([
    midLon, minLat,  // mid lon, min lat
    maxLon, midLat   // max lon, mid lat
  ]);
  
  // Clip state geometry to create four quarters
  var nwQuarter = geometry.intersection(nwBounds);
  var neQuarter = geometry.intersection(neBounds);
  var swQuarter = geometry.intersection(swBounds);
  var seQuarter = geometry.intersection(seBounds);
  
  return [nwQuarter, neQuarter, swQuarter, seQuarter];
}
...
// note function is called processHalf though now processed by quarters as halves had memory errors
function processStateCurrentOnly(state) {
  ...
  if (shouldSplit) {
    print('Splitting', stateName, 'into four quarters');
    var quarters = splitStateIntoQuarters(state);
    
    // Process each quarter
    var quarterNames = ['NW', 'NE', 'SW', 'SE'];
    for (var q = 0; q < 4; q++) {
      var quarterState = ee.Feature(state).setGeometry(quarters[q]);
      processHalf(quarterState, stateName.getInfo() + '_' + quarterNames[q], plantingDOY);
    }
  } else {
    // Process whole state
